...
public class BaseAccountTransferTest {
  ...
  @BeforeEach
  public void setup() {
    // We could either use the @Mock annotations above, or we could instantiate mocks manually:
...
//    smsServiceMock = mock(SmsService.class);
    ...
    /*
    In the following two lines, notice that I have added lenient() in front of when().
    Apparently, because of the @ExtendWith(MockitoExtension.class) in line 16 before the
    class declaration, Mockito uses something called Strict Stubbing. That means, it tries
    to detect common problems. One common problem is "unused stubs".

    Try to remove the lenient() below, and run the tests. They will fail with an error (read the error message!),
    because Mockito thinks this as an "unused stub", since "getAccount" is not called later
    in THIS method. Since Mockito was wrong this time -- we actually WANT to mock these two
    getAccount calls -- we add "lenient()." before when(), to make it relax a bit.
     */
